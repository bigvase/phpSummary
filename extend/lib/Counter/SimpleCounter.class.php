...
class SimpleCounter extends Counter{
	...
	public function desc($childKey, $step = 1) {//使用该函数 过期时间会失效
		...
		return $this->instance->decr($childKey, $step);
	}
	...
	public function set($childKey, $num) {
		if (!is_numeric($num))	throw new \Exception('$num参数必须是数字');
	...
	public function delKey($childKey) {
		return $this->instance->del($childKey);
	}

    public function ttl($childKey) {
        return $this->instance->ttl($childKey);
    }
