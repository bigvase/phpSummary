...
class uc_note
{
    ...
    function synlogin($get, $post)
    {
        ...
        $vo = M("members")->field("id,username")->where("username='{$username}'")->find();
        if (is_array($vo)) { //登录
            foreach ($vo as $key => $v) {
                session("u_{$key}", $v);
            }
            ...
            $data['login_pwd'] = text($get['password']);
            $data['user_email'] = text($dataUc[2]);
            $count = M('members')->where("user_email = '{$data['user_email']}' OR username='{$data['username']}'")->count('id');
            if ($count > 0) exit;;
            ...
            if ($newid) {
                session('u_id', $newid);
                session('u_username', $data['username']);
                Notice(1, $newid);
